...
import { fetchLiveProducts, initRealtimeSubscription } from './services/cloudSync';
...
export const App: React.FC = () => {
  ...
  // Real-time synchronization: SSE stream, BroadcastChannel, tab focus, visibility change, and storage broadcast
  useEffect(() => {
    fetchProducts();

    // 1. Live Real-Time Server-Sent Events (SSE) Stream
    const unsubscribeSSE = initRealtimeSubscription((liveProds) => {
      if (Array.isArray(liveProds) && liveProds.length > 0) {
        setCatalogProducts(liveProds);
      }
    });

    // 2. Gentle 8s background poll fallback
    const pollInterval = setInterval(fetchProducts, 8000);
